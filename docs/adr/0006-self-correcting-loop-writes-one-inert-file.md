...
- **A candidate is validated before it costs anything**: placeholders present, and one `str.format()` with dummy values. A stray brace or a truncated answer is caught in a millisecond instead of after a full corpus regeneration. A rejected candidate still spends its turn — a model that cannot produce a valid template rarely does better on the retry.
