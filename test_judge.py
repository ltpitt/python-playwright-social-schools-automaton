...
from evaluate_digests import apply_rescues
from judge import _key, build_prompt, load_cache, parse_verdicts, save_cache, verdicts
...
class TestVerdicts:
    def test_no_phrases_means_no_call(self):
        with patch("judge._ask", side_effect=AssertionError("must not be called")):
    ...
    def test_a_fresh_verdict_is_cached(self):
        cache = {}
        with patch("judge._ask", return_value='{"1": true}'):
    ...
    def test_only_the_uncached_phrases_are_asked_about(self):
        cache = {_key("m", "digest", "towel"): True}
        with patch("judge._ask", return_value='{"1": false}') as ask:
    ...
    def test_the_digest_schema_is_off_while_judging(self):
        ...
        with patch("judge.get_provider", return_value=FakeProvider()):
...
class TestApplyRescues:
    ...
    def test_a_rescue_can_turn_a_failing_case_into_a_passing_one(self):
        from evaluate_digests import case_passed
...
def test_cache_file_is_json_serialisable(tmp_path):
    cache = {}
    with patch("judge._ask", return_value='{"1": true, "2": false}'):
