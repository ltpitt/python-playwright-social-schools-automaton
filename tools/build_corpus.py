...
from socialschools.paths import CORPUS_FILE, PROCESSED_CORPUS_ARTICLES_FILE, ensure_parent
from socialschools.scraping.attachments import process_links
from socialschools.scraping.browser import resolve_browser_executable_path
from socialschools.scraping.feed import (
    get_article_id,
    get_post_date,
    expand_full_text,
...
DEFAULT_OUT = CORPUS_FILE
DEFAULT_PROCESSED = PROCESSED_CORPUS_ARTICLES_FILE


def collect_article(context, article, with_attachments):
    ...
    attachments = []
    if with_attachments:
        for selector, filetype in (("a[href*='.pdf']", "pdf"), ("a[href*='.docx']", "docx")):
            links = article.query_selector_all(selector)
            if links:
                attachments.extend(process_links(context, links, filetype))
        ...
        try:
            ...
            for article in candidates:
                ...
                if article_id in processed_ids:
                    continue
                case = collect_article(context, article, with_attachments)
                if case:
                    cases.append(case)
        finally:
            browser.close()
...
def update_corpus(out, processed_path, limit, with_attachments):
    ...
    with open(ensure_parent(processed_path), "w", encoding="utf-8") as f:
